...
import json
from datamodel import Listing, Observation, ProsperityEncoder, Trade  # already imported the rest
...
PARAMS = {
    Product.RESIN: {
        # for taking (exploiting obvious mispricings in the order book)
        "fair_value": 10000,
        "take_width": 1,        # how far market value must be from fair value to take the order (so buy @ <= 9.999 / sell @ >= 10.001)    
        "clear_width": 0,       # the width at which we clear our position (e.g. sell at fair_value + clear_width, so 10.000 + 0)
        # for making (market-making)
