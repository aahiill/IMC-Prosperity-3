...
PARAMS = {
    Product.SPREAD: {
        "default_spread_mean": 0,                  # Start with 0 if unknown
        "default_spread_std": 30,                  # Tune based on early volatility
        "spread_sma_window": 100,                  # 1500 is too slow for early learning
        "spread_std_window": 40,                   # 40-50 is a good short-term window
        "zscore_threshold": 2,                   # Use 2–3 instead of 9 for sensitivity
        "target_position": 30,                     # Smaller size until confidence builds
    }
}
...
class Trader:
    ...
    def get_synthetic_basket_order_depth(self, order_depths: Dict[str, OrderDepth]) -> OrderDepth:
        ...
        implied_bid = chocolate_best_bid * CHOCOLATE_PER_BASKET + strawberries_best_bid * STRAWBERRIES_PER_BASKET
        implied_ask = chocolate_best_ask * CHOCOLATE_PER_BASKET + strawberries_best_ask * STRAWBERRIES_PER_BASKET
        ...
        if implied_bid > 0:
            ...
            implied_bid_volume = min(chocolate_bid_volume, strawberries_bid_volume)
        ...
        if implied_ask < float('inf'):
            ...
            implied_ask_volume = min(chocolate_ask_volume, strawberries_ask_volume)
