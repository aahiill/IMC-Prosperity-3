...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        basket1_depth = state.order_depths.get(Product.BASKET1)
        croiss_depth = state.order_depths.get(Product.CROISSANTS)
        jams_depth = state.order_depths.get(Product.JAMS)
        djembes_depth = state.order_depths.get(Product.DJEMBES)

        if all([basket1_depth, croiss_depth, jams_depth, djembes_depth]):
            basket1_mid = get_mid(basket1_depth)
            croiss_mid = get_mid(croiss_depth)
            jams_mid = get_mid(jams_depth)
            djembes_mid = get_mid(djembes_depth)
